...
DATA_FILE = '/var/www/html/neri/api/activity.json'
AUDIO_ROOT = '/var/www/html/neri/audio'
WEB_ROOT = '/var/www/html/neri'
PORT = 9099
...
def geolocate(ip):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'NeriTracker/1.0'})
...
STATE_DIR = '/var/www/html/neri/api'
VALID_IDENTITIES = {'neri', 'ivri', 'boomerim'}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_audio_upload(self):
        ...
        # Check if it's already MP3 (starts with ID3 or ff fb sync bytes)
    ...
    def _trim_silence(self, filepath):
        ...
        try:
            # silenceremove: strip leading silence (start_periods=1), then reverse,
            # strip again (trailing silence), then reverse back
            subprocess.run([
                'ffmpeg', '-y', '-i', filepath,
    ...
    def _handle_praise_edit(self):
        ...
        import re
        data = self._read_body_json()
        array_name = data.get('array', '')  # e.g. CORRECT_TEXTS
        idx = data.get('index', -1)  # 0-based
        ...
        with open(appfile, 'r', encoding='utf-8') as f:
            content = f.read()
        # Find the array and replace the idx-th string
        # Pattern: find "const ARRAY_NAME = [" then count strings
        ...
        if idx < len(strings):
            ...
            # Append new entry at the end of the array
    ...
    def _handle_praise_delete(self):
        ...
        # Parse path like "neri-praise/correct-letter/3.mp3"
        m = _re.match(r'^(.+/)(\d+)\.mp3$', rel_path)
        if not m:
            self._respond_json({'ok': False, 'error': 'invalid path'})
            return
        folder_rel = m.group(1)  # "neri-praise/correct-letter/"
        num = int(m.group(2))
        ...
        if not os.path.exists(target):
            ...
            return
        os.remove(target)
        # Renumber: shift all files after the deleted one down by 1
    ...
    def _handle_milestones(self):
        """Read or edit milestones in app.js."""
        import re
        appfile = os.path.join(WEB_ROOT, 'app.js')
        if self.command == 'GET':
            with open(appfile, 'r', encoding='utf-8') as f:
    ...
    def _handle_praise_texts(self):
        """Read praise text arrays from app.js and return as JSON."""
        import re
        appfile = os.path.join(WEB_ROOT, 'app.js')
        with open(appfile, 'r', encoding='utf-8') as f:
            content = f.read()
